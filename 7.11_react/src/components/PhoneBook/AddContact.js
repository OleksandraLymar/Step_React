...
class AddContact extends React.Component {
    ...
    /**
     * Метод, который делает валидацию
     */
    validate(){
            let err = []
        if(this.state.name.length===0)err.push("The name is too short!")
        if(this.state.subName.length===0)err.push("Enter Surname!")
        if(this.state.number.length===0)err.push("Enter number of your phone!")
    ...
    render() {
        return(
            <>
                <button type="button" className="btn btnMain" data-bs-toggle="modal" data-bs-target="#addContact">
                    Add
                </button>
